...
class TerminalActivityController extends Controller
{
    ...
    public function store(Request $request)
    {
          try {
            //code...
            $request->validate([
                'container_number' => 'required|exists:containers,nomor_container',
...
{
    try {
        $activity = TerminalActivity::where('container_number', $no_container)->firstOrFail();
        ...
        $alreadyRecorded = TerminalActivity::where('container_number', $request->no_container)
            ->whereNotNull('masuk')
            ->whereNotNull('keluar')
            ->exists();
        ...
        if ($alreadyRecorded) {
            return response()->json([
                'success' => false,
                'message' => 'Kendaraan dengan plat ' . $request->no_container . ' sudah tercatat masuk & keluar, tidak bisa disimpan lagi.',
            ], 422);
        }
